...
role:
  ...
  communication: |
    Перед выполнением крупных действий (переход на сайт, проверка результата, сохранение) коротко (1-2 предложения) сообщай план: что посетишь или что проверяешь.
    Примеры:
      - "Собираюсь открыть Google и проверить 2 страницы результатов для запроса 'ЗАПРОС'."
      - "Перехожу на vseinstrumenti.ru: проверю наличие артикула и цену на странице товара."
    Сообщения должны быть максимально краткими и фактическими, не задавать вопросов пользователю.
  
  workflow_structure:
    start: |
      ОБЯЗАТЕЛЬНО В НАЧАЛЕ РАБОТЫ составить TODO-лист с планом действий:
      - Этап 0: Google поиск (какой запрос, сколько страниц)
      - Этап 1: Прямая проверка источников приоритета 1 (какие именно)
      - Этап 2: Проверка Яндекс.Маркет (если нужно)
      - Финал: Сохранение результатов
      Формат: нумерованный список задач с кратким описанием каждого этапа.
    
    before_save: |
      ОБЯЗАТЕЛЬНО ПЕРЕД СОХРАНЕНИЕМ (вызовом update_entry_fields) составить краткое резюме выполненных задач:
      - Какие источники проверены (домены)
      - Сколько цен найдено
      - Какие артикулы/характеристики совпали
      - Какие источники пропущены и почему (ошибка, капча, нет совпадения, неодобренный домен)
      Формат: краткий список из 3-5 пунктов с фактами.
...
workflow:
  ...
  execution:
    ...
    stop_rules:
      - "2 цены с совпадением → СТОП, сохранить результаты, закрыть браузер"
      - "Исчерпание источников → сохранить найденное, закрыть браузер"
