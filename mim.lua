...
role:
  ...
  mandatory_workflow:
    step_1_todo_list: |
      ...
      1. ⏳ Шаг 1: Google поиск - запрос '[ЗАПРОС]' (только первая страница)
      2. ⏳ Шаг 2: Проверка ВСЕХ найденных источников из Google (не входят в лимит)
      3. ⏳ Шаг 3: Проверка остальных источников из списка (макс. 6 прямых источников)
      4. ⏳ Финал: Сохранение результатов и закрытие браузера
      
      ⚠️ Лимит: результаты Google + максимум 6 прямых источников!"
...
search_strategy: &strategy
  step_1: "Google поиск (только первая страница) → отбор ссылок с одобренных доменов"
  step_2: "Проверка ВСЕХ найденных через Google источников (не входят в лимит 6)"
  step_3: "Проверка оставшихся источников из списка по порядку (максимум 6)"
  max_sources: "Максимум 6 прямых источников + все найденные через Google"
  stop_condition: "Товар найден ИЛИ исчерпаны все источники (Google + 6 прямых)"


automation_rules:
  - "НЕ задавать вопросы - принимать решения самостоятельно"
  - "Автоматически выбирать оптимальную стратегию поиска"
  - "ЛИМИТ ИСТОЧНИКОВ: ВСЕ результаты Google + максимум 6 прямых источников"
  - "КРИТЕРИЙ ОСТАНОВКИ: товар найден ИЛИ исчерпаны источники (Google + 6 прямых)"
  - "Сохранять результаты после завершения поиска (найден товар или достигнут лимит)"
...
workflow:
  ...
  execution:
    google_search:
      action: "Playwright → Google (только первая страница) → отбор по доменам → Playwright проверка ВСЕХ"
      ...
      note: "ВСЕ найденные через Google ссылки проверяются БЕЗ лимита"
      domain_filter: |
        ...
        - Приоритет: проверять ВСЕ источники, найденные через Google
    ...
    direct_search:
      ...
      priority: "15 основных источников (проверять по порядку)"
      limit: "СТРОГИЙ ЛИМИТ: максимум 6 прямых источников (НЕ считая проверенные через Google)"

    stop_rules:
      - "Товар найден (цена и артикул совпали) → СТОП, сохранить результаты, закрыть браузер"
      - "Проверены ВСЕ источники Google + 6 прямых → СТОП, сохранить найденное (или пусто), закрыть браузер"
