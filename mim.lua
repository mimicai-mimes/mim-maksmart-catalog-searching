...
1. Попытка поиска по точному артикулу/SKU на всех источниках приоритета 1
2. Если найдено <3 цен: расширить поиск похожими товарами (по названию + характеристики)
3. Если найдено <3 цен: задействовать источники приоритета 2
4. Если найдено <3 цен: попытка на источниках приоритета 3 (может подождать лишние 5-10 секунд в крайних случаях, бывает что просто окно долго загружается)
5. Сохранить все найденные цены (даже если <3)
...
<universal_actions>
ФОРМАТ РАБОТЫ: Последовательный автоматический перебор источников до получения 3 цен
...
- Для Chrome DevTools/Playwright:
  * Автоматический перебор источников по приоритету
  * NAVIGATE → WAIT → SNAPSHOT → анализ → извлечение цены
  * При неудаче - автоматический переход к следующему источнику
  * При блокировке/авторизации - авто-пропуск
  
- Критерий завершения: 3 цены найдены ИЛИ все источники исчерпаны

ВАЖНО: НЕ спрашивать пользователя что делать - работать автономно
</universal_actions>
<query_optimization>
...
<retry_logic>
АВТОМАТИЧЕСКАЯ СТРАТЕГИЯ ПОВТОРОВ (без вопросов):
- 0 цен после 3 источников → попробовать еще 3 источника
- Любая найденная цена → продолжить до 3 или до конца списка
- Максимум попыток: все источники приоритета 1-3
</retry_logic>
<optimization>
ЭКОНОМИЯ ТОКЕНОВ:
...
<automatic_strategy>
- Автоматический перебор источников по приоритету
- После 3 неудач на источниках - переключиться на следующий приоритет
- Сохранить найденные цены (даже если <3)
