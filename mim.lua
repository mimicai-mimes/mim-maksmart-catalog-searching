...
workflow:
  step_1:
    ...
    automatic_workflow:
      ...
      stage_0:
        name: "GOOGLE ПОИСК БЕЗ ФИЛЬТРА"
        description: "быстрая предварительная проверка, 2 страницы"
        
        step_1:
          action: "Сформировать Google запрос БЕЗ фильтра site: (фильтр не работает)"
          query_format: "НАЗВАНИЕ_ТОВАРА АРТИКУЛ купить"
        
        step_2:
          action: "Выполнить через Playwright MCP (проверка 2 страниц)"
          page_1:
            required: true
            actions:
          ...
          page_2:
            ...
            actions:
              ...
              - "Отфильтровать по одобренным доменам и типу (только товары)"
            skip_condition: "Если уже найдено ≥5 ссылок → пропустить страницу 2 (экономия времени)"
          
          cleanup:
            - "browser_close()"
        
        step_3:
          action: "Отфильтровать результаты по одобренным доменам"
          tasks:
            - "Извлечь ВСЕ ссылки из snapshot (со страницы 1 и 2)"
            - task: "Отобрать ТОЛЬКО ссылки на товары с одобренных доменов"
              domains:
        ...
        step_4:
          action: "Проверить отобранные товары через fetch"
          for_each_link:
            - 'fetch([url_товара], "цена, артикул, название товара")'
          ...
          outcomes:
            - condition: "Если найдено 2 цены с совпадающими характеристиками"
              action: "СТОП, переход к сохранению"
            - condition: "Если найдено <2 цен"
              action: "запомнить уже проверенные сайты, переход к ЭТАПУ 1"
      
      stage_1:
        name: "ПРЯМАЯ ПРОВЕРКА ОСНОВНЫХ ИСТОЧНИКОВ"
        condition: "если не хватило из Google"
        steps:
          - "Последовательно проверять источники приоритета 1, ПРОПУСКАЯ уже проверенные через Google"
          - 'Каждый источник проверяется как описано в substep "direct_search"'
          - "Если найдено 2 цены с совпадающими характеристиками → НЕМЕДЛЕННО СТОП, сохранить, завершить"
          - "Если найдено <2 цен после проверки всех доступных источников приоритета 1 → ЭТАП 2"
      
      stage_2:
        name: "ПРОБЛЕМНЫЕ ИСТОЧНИКИ"
        condition: "если всё ещё не хватило"
        steps:
          - "Проверить источники приоритета 2 (максимум 2 шт, с авто-пропуском блокировок)"
          - "Если найдено 2 цены с совпадающими характеристиками → НЕМЕДЛЕННО СТОП, сохранить, завершить"
          - "После проверки последнего источника → ОБЯЗАТЕЛЬНОЕ ЗАВЕРШЕНИЕ"
          - "Сохранить все найденные цены (даже если 0, 1)"
  ...
  step_2:
    ...
    retry_logic:
      ...
      stage_0:
        title: "ЭТАП 0 - Google БЕЗ фильтра с отбором (быстрая предпроверка, 2 страницы)"
        process:
      ...
      stage_1:
        title: "ЭТАП 1 - Прямая проверка (пропуская проверенные)"
        process:
    ...
    automatic_strategy:
      ...
      stage_2:
        ...
        steps:
          ...
          - completion_rules:
              title: "Завершить поиск ОБЯЗАТЕЛЬНО когда"
              conditions:
