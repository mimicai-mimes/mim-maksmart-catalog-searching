...
automation_rules:
  ...
  - "Закрывать браузер Playwright перед сохранением результатов"
...
tools:
  playwright_mcp:
    ...
    methods:
      ...
      close: "#browser_close - Закрытие после сохранения (если не получается, то пропустить шаг)"
    patterns:
...
workflow:
  ...
  execution:
    ...
    stop_rules:
      - "2 цены с совпадением → СТОП, сохранить, закрыть браузер"
