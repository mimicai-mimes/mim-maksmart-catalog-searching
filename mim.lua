...
role:
  ...
  mandatory_workflow:
    step_1_todo_list: |
      ...
      2. ⏳ Шаг 2: Анализ выдачи - есть ли приоритетные источники (из 16)?
      3. ⏳ Шаг 3: Проверка найденных приоритетных источников в порядке приоритета
      4. ⏳ Шаг 4: Если не хватает цен → проверка 6 дополнительных источников
...
search_strategy: &strategy
  step_1: "Google поиск (первая страница) → анализ выдачи на наличие приоритетных источников"
  step_2: "Если есть приоритетные в Google → проверка в порядке приоритетности (из 16 источников)"
  step_3: "Если нет приоритетных в Google ИЛИ не хватает цен → проверка 6 дополнительных источников"
...
automation_rules:
  ...
  - "ПРИОРИТЕТНЫЕ ИСТОЧНИКИ: 16 источников - проверять в порядке приоритетности если найдены в Google"
  - "ДОПОЛНИТЕЛЬНЫЕ ИСТОЧНИКИ: 6 источников - использовать если не хватает цен после приоритетных"
...
workflow:
  ...
  execution:
    ...
    additional_sources_check:
      action: "Проверка 6 дополнительных источников (НЕ найденных в Google)"
      pattern: "navigate(search_url) → snapshot → evaluate(товары и цены)"
      limit: "Максимум 6 дополнительных источников (пропуская уже проверенные)"
      priority: "Проверять в порядке приоритетности из списка approved_domains"
      stop_condition: "Найдено 2 цены ВСЕГО (приоритетные + дополнительные) → СТОП"
...
# РЕЗУЛЬТАТ
expected_result:
  goal: "Найти 2 цены для товара из любой комбинации источников (приоритетные + дополнительные)"
  mode: "Автономный с приоритизацией источников"
  optimization: "Google поиск → приоритетные источники → дополнительные источники (до 6)"
